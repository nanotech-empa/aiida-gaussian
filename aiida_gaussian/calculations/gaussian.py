...
class GaussianCalculation(CalcJob):
    ...
    def _render_input_string_from_params(cls, param_dict, structure):
        ...
        # In the following, the param_dict.get() method is used,
        # which returns `None` if the corresponding key is not specified,
        # resulting in the default value defined by the pymatgen library.
        # For example, charge and spin_multiplicity are then defined
        # based on the pymatgen molecule definition.
