...
class GaussianCalculation(CalcJob):
    ...
    def _render_input_string_from_params(cls, parameters, pmg_structure):
        ...
        if (
            parameters.get("basis_set", "").lower() == "genecp"
            and "gen_basis" in parameters
        ):
            # The ``GaussianInput`` class automatically sets ``basis_set`` to ``Gen`` if ``gen_basis`` is specified.
            # However, if the user explicitly set ``basis_set`` to ``genecp``, we should respect that, since if the
            # custom basis set definition contains effective core potentials, running with ``genecp`` is significantly
            # faster than just running with ``gen``.
            gaussian_input.basis_set = parameters["basis_set"]
